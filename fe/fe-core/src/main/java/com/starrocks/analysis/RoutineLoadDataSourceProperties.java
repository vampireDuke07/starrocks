...
public class RoutineLoadDataSourceProperties {
    ...
    private void checkPulsarProperties() throws AnalysisException {
        Optional<String> optional = properties.keySet().stream().filter(
                entity -> !CONFIGURABLE_PROPERTIES_SET_PULSAR.contains(entity)).filter(
                entity -> !entity.startsWith("property.")).findFirst();
        if (optional.isPresent()) {
        ...
        // check custom properties
        CreateRoutineLoadStmt.analyzeCustomProperties(properties, customPulsarProperties);
    }
    ...
    public String toString() {
        ...
        sb.append(", kafka custom properties: ").append(customKafkaProperties);
        sb.append(", pulsar partition offsets: ").append(pulsarPartitionOffsets);
        sb.append(", pulsar custom properties: ").append(customPulsarProperties);
        return sb.toString();
    }
}
