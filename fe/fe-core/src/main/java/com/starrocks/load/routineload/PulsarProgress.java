...
public class PulsarProgress extends RoutineLoadProgress {
    ...
    public PulsarProgress(TKafkaRLTaskProgress tKafkaRLTaskProgress) {
        super(LoadDataSourceType.PULSAR);
        //todo tKafkaRLTaskProgress to tPulsarRLTaskProgress
    ...
    public void convertOffset(String brokerList, String topic, Map<String, String> properties) throws UserException {
        ...
        if (endPartitions.size() > 0) {
            Map<Integer, Long> partOffsets =
                    PulsarUtil.getLatestOffsets(brokerList, topic, ImmutableMap.copyOf(properties), endPartitions);
            partitionIdToOffset.putAll(partOffsets);
        }
    }
