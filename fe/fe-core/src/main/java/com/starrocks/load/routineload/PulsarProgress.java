...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/load/routineload/PulsarProgress.java
...
public class PulsarProgress extends RoutineLoadProgress {
    ...
    public PulsarProgress(TPulsarRLTaskProgress tPulsarRLTaskProgress) {
        super(LoadDataSourceType.PULSAR);
        this.partitionIdToOffset = tPulsarRLTaskProgress.getPartitionCmtOffset();
    }
    ...
    public Long getOffsetByPartition(int pulsarPartition) {
        return partitionIdToOffset.get(pulsarPartition);
    }

    public boolean containsPartition(Integer pulsarPartition) {
        return partitionIdToOffset.containsKey(pulsarPartition);
    }
    ...
    public void modifyOffset(List<Pair<Integer, Long>> pulsarPartitionOffsets) throws DdlException {
        for (Pair<Integer, Long> pair : pulsarPartitionOffsets) {
    ...
    public void convertOffset(String serverUrl, String topic, Map<String, String> properties) throws UserException {
        ...
        if (beginningPartitions.size() > 0) {
            Map<Integer, Long> partOffsets = PulsarUtil
                    .getBeginningOffsets(serverUrl, topic, ImmutableMap.copyOf(properties), beginningPartitions);
            partitionIdToOffset.putAll(partOffsets);
        }
        if (endPartitions.size() > 0) {
            Map<Integer, Long> partOffsets =
                    PulsarUtil.getLatestOffsets(serverUrl, topic, ImmutableMap.copyOf(properties), endPartitions);
            partitionIdToOffset.putAll(partOffsets);
        }
    }
    ...
    public String toString() {
        ...
        return "PulsarProgress [partitionIdToOffset="
